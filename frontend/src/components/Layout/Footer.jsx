...
import { footerCompanyLinks, footerProductLinks, footerSupportLink } from '../../static/data'
...
const Footer = () => {
  ...
  return (
    <div className='bg-[#000] text-white'>
        <div className='md:flex md:justify-between md:items-center sm:px-12 bg-[#342ac8] py-7'>
            <h1 className='lg:text-4xl text-3xl md:mb-0 mb-6 lg:leading-normal font-semibold md:w-2/5'><span className='text-[#56d879]'>Subcribe</span> us for get news{" "}
            <br />
            events and offers
            </h1>
            <div>
        ...
        <div className='grid grid-cols-1 sm:grid-cols-3 lg:grid-cols-4 gap-6 sm:px-8 px-5 py-16 sm:text-center'>
            ...
            <ul className='text-center sm:text-start'>
                ...
                {
                    footerSupportLink.map((link, index) => (
                        <>
                            <li key={index}>
                                <Link to={link.link} className='text-gray-400 hover:text-teal-400 duration-300 text-sm cursor-pointer leading-6'>
                                    {link.name}
